...
from connectors.stripe_connector import StripeFitnessConnector

@retry_with_backoff(max_retries=3)
def sync_mindbody(company_id: str, site_id: str, api_key: str):
    """Syncs Mindbody data including members, attendance, and memberships."""
    if not supabase:
        return
        
    print(f"Syncing Mindbody for {company_id}...")
    connector = MindbodyConnector(company_id, site_id=site_id, api_key=api_key)
    result = connector.full_sync()
    
    if result.success:
        # Save results summary to insights
        supabase.table("insights_core").insert({
            "company_id": company_id,
            "insight_type": "connector_sync",
            "insight_value": result.members_synced + result.attendance_synced,
            "metadata": result.to_dict()
        }).execute()
        
    print(f"Mindbody sync complete: {result.to_dict()}")
    return result

@retry_with_backoff(max_retries=3)
def sync_glofox(company_id: str, api_key: str, api_secret: str, branch_id: str):
    """Syncs Glofox data for EU fitness businesses."""
    if not supabase:
        return
        
    print(f"Syncing Glofox for {company_id}...")
    ...
    print(f"Glofox sync complete: {result.to_dict()}")
    return result

@retry_with_backoff(max_retries=3)
def sync_google_calendar_fitness(company_id: str, credentials_path: str):
    """Syncs class schedules and attendance from Google Calendar."""
    if not supabase:
        return
        
    print(f"Syncing Google Calendar (Fitness) for {company_id}...")
    ...
    print(f"Google Calendar fitness sync complete: {result.to_dict()}")
    return result
...
def sync_stripe_fitness(api_key: str, company_id: str):
    """
    Syncs Stripe subscriptions specifically for Fitness/Wellness businesses.
    Maps subscriptions to Tactics Membership models for LTV analysis.
    """
    if not supabase:
        return
        
    print(f"Syncing Stripe (Fitness) for {company_id}...")
    connector = StripeFitnessConnector(company_id, api_key=api_key)
    result = connector.full_sync()
    
    if result.success:
        # For Stripe fitness, we also save members as clients and memberships as sales
        members = connector.get_members()
        memberships = connector.get_memberships()
        
        # 1. Sync Members to Clientes
        client_rows = [m.to_tactics_row() for m in members]
        for row in client_rows:
            row["company_id"] = company_id
            supabase.table("clientes").upsert(row).execute()
            
        # 2. Sync Memberships to Ventas
        sale_rows = [m.to_tactics_row() for m in memberships]
        for row in sale_rows:
            row["company_id"] = company_id
            # In a real app, we would match customer_id from internal CLI_ mapping
            supabase.table("ventas").upsert(row).execute()
            
        supabase.table("insights_core").insert({
            "company_id": company_id,
            "insight_type": "connector_sync",
            "insight_value": len(memberships),
            "metadata": result.to_dict()
        }).execute()
        
    print(f"Stripe fitness sync complete: {result.to_dict()}")
    return result
